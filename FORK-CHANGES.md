...
2. 同时在**后台**异步（不阻塞启动、不打扰 TUI）：`git fetch upstream main` → 算
   `main..upstream/main` 落后数 → 用 `git merge-tree --write-tree`（内存试合并，不碰
   工作区/索引）预检合并是否冲突；结果写入 `~/.cache/codex-fold/upstream-behind`，
   格式为 `<落后数> <clean|conflict>`（覆盖写，不累加，是当前状态快照）；
3. 本次退出后（已回到主屏幕），若上一次检查发现上游有新提交，打印一行黄色提示：
   - 可干净合并 → 建议 `rebuild-codex-fold --sync`；
   - 有冲突 → 提示 `--sync` 会中止合并，需手动 / AI 解决冲突。
